...
> ⚠️ **安全警告**：默认配置 `token = "qrmai"` 且 `host = "0.0.0.0"`（局域网可访问）。
> **部署到任何可被他人访问的网络前，请务必修改 `config.toml` 中的 token！**
...
浏览器访问 `http://127.0.0.1:5000`，使用令牌登录（默认 `qrmai`，请修改！）。
